...
describe('Bisect the bug', () => {
  beforeEach(() => {
    document.querySelectorAll('style').forEach(s => s.remove())
  })

  // TEST C passes, TEST D fails. Differences:
  // 1. --scheme in first vs second :root
  // 2. --ink-base
  // 3. --l-layer-quiet/loud (not just mid)
  // 4. --border
  // 5. --i0 (--tint)
  // 6. --g-layer-b, --g-layer-c

  it('TEST E: Add --ink-base only', () => {
  ...
  it('TEST F: Add --l-layer-quiet/loud', () => {
    ...
    document.head.appendChild(style)
    const root = getComputedStyle(document.documentElement)
    console.log('TEST F (add --l-layer-quiet/loud):')
    console.log('  --intent-color:', root.getPropertyValue('--intent-color').trim())
    expect(root.getPropertyValue('--intent-color').trim()).not.toBe('')
  })
})
