...
describe('Test fix: use if() instead of --pick for ink', () => {
  beforeEach(() => {
    document.querySelectorAll('style').forEach(s => s.remove())
  })

  it('TEST N: --ink-base using if() directly (no --pick)', () => {
    ...
    style.textContent = `
      ...
      @layer tokens {
        ...
        :root {
          /* FIX: Use if() directly instead of --neutral(--pick(...)) */
          --ink-base: if(
            style(--scheme: dark): --neutral(var(--l-ink-dark));
            else: --neutral(var(--l-ink-light));
          );
    ...
    console.log('TEST N (if() instead of --pick for ink):')
