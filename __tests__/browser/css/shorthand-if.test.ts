...
describe('Shorthand if() syntax tests', () => {
  beforeEach(() => {
    document.querySelectorAll('style').forEach(s => s.remove())
  })

  it('TEST 81: Shorthand if() for --layer-base (no nesting)', () => {
    ...
    style.textContent = `
      ...
      @layer tokens {
        ...
        @function --emph() returns <percentage> {
          result: if(
            style(--emphasis: loud): 12%;
            style(--emphasis: quiet): 4%;
            else: 8%;
          );
        }
        ...
        :root {
          ...
          /* SHORTHAND if() - no nesting */
          --layer-base: if(
            style(--emphasis: loud): --neutral(var(--l-layer-loud));
            style(--emphasis: quiet): --neutral(var(--l-layer-quiet));
          ...
          --intent-src: if(
            ...
            style(--intent: destructive): var(--intent-destructive);
            else: var(--layer-base);
          );

          /* SHORTHAND if() - no nesting */
          --intent-boost: if(
            style(--emphasis: loud): 18%;
            style(--emphasis: quiet): 8%;
            else: 12%;
          );
    ...
    console.log('TEST 81 (shorthand if() syntax):')
  ...
  it('TEST 82: Compare with nested if() (original failing pattern)', () => {
    const style = document.createElement('style')
    style.textContent = `
      @layer tokens, base, system;
      ...
      @layer tokens {
        ...
        :root {
          ...
          /* NESTED if() - original pattern */
          --layer-base: if(
            style(--emphasis: loud): --neutral(var(--l-layer-loud)); else:
              if(style(--emphasis: quiet): --neutral(var(--l-layer-quiet)); else: --neutral(var(--l-layer-mid));)
          );

          /* NESTED if() - original pattern */
          --intent-src: if(
            style(--intent: accent): var(--intent-accent); else:
              if(style(--intent: positive): var(--intent-positive); else:
                if(style(--intent: caution): var(--intent-caution); else:
                  if(style(--intent: destructive): var(--intent-destructive); else: var(--layer-base);)
                )
              )
          );
          ...
          /* NESTED if() - original pattern */
          --intent-boost: if(
            style(--emphasis: loud): 18%; else:
              if(style(--emphasis: quiet): 8%; else: 12%;)
          );
    ...
    document.head.appendChild(style)
    const root = getComputedStyle(document.documentElement)
    console.log('TEST 82 (nested if() syntax - original):')
    console.log('  --layer-base:', root.getPropertyValue('--layer-base').trim())
    console.log('  --intent-src:', root.getPropertyValue('--intent-src').trim())
    console.log('  --intent-color:', root.getPropertyValue('--intent-color').trim())
    console.log('  --g-layer-a:', root.getPropertyValue('--g-layer-a').trim())
    // This one may fail - it's the original pattern
    expect(root.getPropertyValue('--layer-base').trim()).not.toBe('')
  })
})
