...
describe('Test: literal --ink-base', () => {
  beforeEach(() => {
    document.querySelectorAll('style').forEach(s => s.remove())
  })

  it('TEST P: --ink-base as literal oklch via if()', () => {
    ...
    style.textContent = `
      ...
      @layer tokens {
        ...
        :root {
          /* FIX: --ink-base as literal oklch with if() - NO function nesting */
          --ink-base: if(
            style(--scheme: dark): oklch(0.92 0 var(--hue));
            else: oklch(0.18 0 var(--hue));
          );
    ...
    console.log('TEST P (literal oklch for --ink-base):')
