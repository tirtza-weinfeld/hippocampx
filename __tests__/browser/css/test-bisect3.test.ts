...
describe('Bisect ink-base vs border', () => {
  beforeEach(() => {
    document.querySelectorAll('style').forEach(s => s.remove())
  })

  it('TEST I: Add --ink-base only (no --border)', () => {
  ...
  it('TEST J: Add --border only (no --ink-base)', () => {
    ...
    style.textContent = `
      ...
      @layer tokens {
        ...
        :root {
          ...
          --intent-boost: if(style(--emphasis: loud): 18%; style(--emphasis: quiet): 8%; else: 12%;);
          --intent-color: --mix(var(--layer-base), var(--intent-src), var(--intent-boost));
          /* --border without --ink-base - needs a literal color */
          --border: --mix(var(--layer-base), oklch(0.18 0 265), 18%);
          --i0: --tint(var(--intent-color), var(--ramp-tint));
          --i1: var(--intent-color);
          --i2: --shade(var(--intent-color), var(--ramp-shade));
          --aura: --emph();
    ...
    document.head.appendChild(style)
    const root = getComputedStyle(document.documentElement)
    console.log('TEST J (--border only, no --ink-base):')
    console.log('  --intent-color:', root.getPropertyValue('--intent-color').trim())
    expect(root.getPropertyValue('--intent-color').trim()).not.toBe('')
  })
})
