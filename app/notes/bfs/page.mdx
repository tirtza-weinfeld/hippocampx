...
### Binary Tree Level Order Traversal
  ...
  *Given the root of a binary tree, return its nodes' values organized by level,
  ...
  [102. Binary Tree Level Order Traversal](https://leetcode.com/problems/binary-tree-level-order-traversal/)
    ...
    ```python file=examples/code/bfs.py#func:levelOrder stripDocstring 
    ```
...
```python file=examples/code/bfs.py#func:numIslands  stripDocstring 
```
...
```python file=examples/code/bfs.py#func:orangesRotting  stripDocstring 
```
...
```python file=examples/code/bfs.py#func:cloneGraph stripDocstring 
```
...
```python file=examples/code/bfs.py#func:snakesAndLadders stripDocstring 
```
...
```python file=examples/code/bfs.py#func:updateMatrix stripDocstring 
```
...
```python file=examples/code/bfs.py#func:shortestPath stripDocstring 
```

> [!insight:collapse] BFS with Optimized State Pruning ⚡
>
> This solution uses a optimized BFS. Instead of just tracking `(row, col, k)` tuples in a set, 
the `visited` array stores the **maximum elimination power** remaining upon arrival at each `(x, y)` cell.
A new path to a cell is only explored if it arrives with *more* power than any previous path (`nk > visited[nx][ny]`). 
This aggressively prunes suboptimal paths.
> The initial check (`m + n - 2 <= k`) is another optimization, instantly solving trivial cases where obstacles don't matter.

> [!timecomplexity:collapse] $O(m \cdot n \cdot k)$
>
> The theoretical worst-case state space is defined by the grid dimensions and `k`. However, the state pruning optimization makes the practical performance much faster than the worst-case for most inputs.
...
```python file=examples/code/bfs.py#func:ladderLength stripDocstring 
```
