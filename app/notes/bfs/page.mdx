...
### 1. Binary Tree Level Order Traversal
  ...
  *Given the root of a binary tree, return its nodes' values organized by level,
  ...
  [102. Binary Tree Level Order Traversal](https://leetcode.com/problems/binary-tree-level-order-traversal/)
    ...
    ```python file=problems/102-binary-tree-level-order-traversal/solution.py  
    ```
...
```python file=problems/200-number-of-islands/bfs.py
```
...
> [\!timecomplexity:collapse] $O(R \cdot C)$
>
...
```python file=problems/994-rotting-oranges/solution.py
```
...
> [\!timecomplexity:collapse] $O(R \cdot C)$
>
> where $R$ and $C$ are the grid dimensions. Each cell is enqueued and dequeued at most once, making the process highly efficient and optimal.
...
```python file=problems/133-clone-graph/solution.py
```
...
```python file=problems/909-snakes-and-ladders/solution.py  
```
...
```python file=problems/542-01-matrix/solution.py  
```
...
```python file=problems/1293-shortest-path-in-a-grid-with-obstacles-elimination/bfs.py:shortest_path_in_a_grid_with_obstacles_elimination 
```
...
```python file=problems/127-word-ladder/solution.py  
```
