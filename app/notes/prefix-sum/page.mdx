...
```python file=problems/325-maximum-size-subarray-sum-equals-k/solution.py
```
...
```python file=problems/560-subarray-sum-equals-k/solution.py
```
...
```python file=problems/525-contiguous-array/solution.py
```
...
```python file=problems/523-continuous-subarray-sum/solution.py
```
...
```python file=problems/974-subarray-sums-divisible-by-k/solution.py
```
...
```python file=problems/1590-make-sum-divisible-by-p/solution.py
```
...
>
> - ~ `a % c == b % c` (Two numbers have the same remainder)
> - ~ `(a - b) % c == 0` (Their difference is a multiple of `c`)
>
>
>
> Let's apply this:
...
> 
> 
> - ~ **Start with the objective:** We need the subarray's sum (`C - P`) to have the remainder `R`.
>     `(C - P) % p == R`
> 
> - ~ **Isolate the unknown:** We need to find the `Past Prefix Sum` (`P`). Using the rules of modular arithmetic, we can rearrange the equation to solve for `P`.
>     `P % p == (C - R) % p`
...
> > **Applying the Rule**
> > 
> > - ~ **Start with the equation in congruence form:**
> >     `(C - P) % p == R` is the same as `C - P ≡ R (mod p)`.
> > 
> > - ~ **Rearrange using algebra:** To solve for `P`, you can add `P` to both sides and subtract `R` from both sides.
> >     `C - R ≡ P (mod p)`
> > 
> > - ~ **Convert back:** :
> >     `P % p == (C - R) % p`
...
> 
> - ~ **Define `need`:** The term we solved for, `Past Prefix Sum % p`, is the value the code is looking for. This is what **`need`** represents.
>     `need = (C - R) % p`
> 
> **Why add `+ p`?**
> 
...
```python file=problems/370-range-addition/solution.py
```
...
```python file=problems/2145-count-the-hidden-sequences/solution.py
```
