...
s
  ...
  * `[python:]def player(board)`: Tells us whose turn it is ('X' or 'O').
...
1.  **The Maximizer:** Player 'X' is the **maximizer**. Their goal is to pick the move that leads to a game state with the highest possible `utility` score.
2.  **The Minimizer:** Player 'O' is the **minimizer**. Their goal is to pick the move that leads to a game state with the lowest possible `utility` score.
