...
```python file=examples/code/dijkstra.py#func:dijkstra 
```
...
```python file=examples/code/dijkstra.py#func:networkDelayTime   
```
...
```python file=examples/code/dijkstra.py#func:minimumEffortPath   
```
...
### 3\. Swim in Rising Water
  ...
  *You are given an `N x N` grid of elevations. Find the minimum "time" `t` to travel from `(0, 0)` to `(N-1, N-1)`. 
    ...
    ```python file=examples/code/dijkstra.py#func:swimInWater   meta="/[2:]grid[0][0]/"
    ```
...
```python file=examples/code/dijkstra.py#func:findCheapestPriceDijkstra  
```
...
```python file=examples/code/dijkstra.py#func:dijkstraShortestPath  
```
