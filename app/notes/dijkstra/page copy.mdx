...
```python file=problems/dijkstra.py:dijkstra
```
...
### 3\. Swim in Rising Water
  ...
  *You are given an `N x N` grid of elevations. Find the minimum "time" `t` to travel from `(0, 0)` to `(N-1, N-1)`. 
    ...
    ```python file=problems/778-swim-in-rising-water/solution.py:swim_in_rising_water   meta="/[2!]grid[0][0]/"
    ```
...
```python file=problems/787-cheapest-flights-within-k-stops/dijkstra.py:cheapest_flights_with_at_most_k_stops_dijkstra  
```
...
```python file=problems/dijkstra_shortest_path.py:dijkstra_shortest_path  
```
