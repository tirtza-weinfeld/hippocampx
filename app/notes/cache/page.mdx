...
This policy is efficiently implemented using an `OrderedDict` in Python, which internally combines a hash map for fast lookups and a doubly-linked list to maintain insertion order.
  ...
  * **`put(key, value)`**: When an item is added or updated, it's also considered the most recently used and is moved to the end. If the cache exceeds its capacity, the item at the front of the `OrderedDict` (the least recently used) is removed.
...
```python file=examples/code/cache.py#class:LRUCache stripDocstring 
```
...
```python file=examples/code/cache.py#class:LFUCache stripDocstring 
```
