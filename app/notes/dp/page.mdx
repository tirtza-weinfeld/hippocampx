...
> [\!Intuition:collapse]
>
> The core insight is that the minimum path to any cell must come from one of the two cells directly below it. The function `dp(r, c)` leverages this by recursively finding the best path from the bottom up. By starting at the top and asking for the solution to the subproblems below (`dp(r+1, ...)`), we find the optimal path. Memoization ensures that the path from any given cell is computed only once.

> [\!timecomplexity:collapse] **$O(n^2)$**
>
> **$O(n^2)$**: where `n` is the number of rows in the triangle. Each state `(r, c)` is computed once.
