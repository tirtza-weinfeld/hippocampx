...
import type { Root, List, ListItem, Paragraph, Text, Blockquote, Heading } from 'mdast'
...
// Also provides context-aware list item rendering based on section headers
const remarkListVariants: Plugin<[], Root> = () => {
  // Track current section context for context-aware list rendering
  let currentContext = 'default'
  
  // Helper function to extract text from heading node
  const extractTextFromHeading = (heading: Heading): string => {
