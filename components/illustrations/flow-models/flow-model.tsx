...
export function FlowModelIllustration({ size = "md" }: { size?: IllustrationSize }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <motion.svg className={sizeClasses[size]} viewBox="0 0 160 105" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        ...
        <motion.g initial={{ opacity: 1 }} animate={{ opacity: phase === "init" ? 1 : 0.4 }} transition={ease}>
        ...
        <motion.g initial={{ opacity: 0.4 }} animate={{ opacity: phase === "sample" ? 1 : 0.4 }} transition={ease}>
        ...
        {/* Neural network box */}
        <motion.g initial={{ opacity: 0.3 }} animate={{ opacity: phase === "network" || phase === "simulate" ? 1 : 0.3 }} transition={ease}>
        ...
        {/* Moving point */}
        <motion.g initial={{ x: 0 }} animate={{ x: (dataX - noiseX) * progress }} transition={ease}>
        ...
        <motion.g initial={{ opacity: 0 }} animate={{ opacity: phase === "network" || phase === "simulate" ? 0.6 : 0 }} transition={ease}>
          {[60, 80, 100].map((x, i) => (
        ...
        {/* Algorithm indicator */}
        <motion.g initial={{ opacity: 0 }} animate={{ opacity: phase === "simulate" || phase === "sample" ? 1 : 0 }} transition={ease}>
