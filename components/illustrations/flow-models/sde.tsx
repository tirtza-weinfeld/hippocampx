  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <motion.svg width="160" height="105" viewBox="0 0 160 105" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <motion.path d={driftD} fill="none" className="stroke-green-500 dark:stroke-green-400" strokeWidth="2" strokeDasharray="4 2" animate={{ opacity: showDrift ? 0.7 : 0 }} transition={ease} />
        {sdePaths.map((d, i) => (
          <motion.path key={i} d={d} fill="none" className="stroke-blue-500 dark:stroke-blue-400" strokeWidth="1.5" animate={{ opacity: showSDE ? 0.8 : 0 }} transition={ease} />
        ))}
