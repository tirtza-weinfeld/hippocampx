            ...
            <DialogPrimitive.Content asChild>
              <motion.div
                className={cn(
                  ...
                  'bg-er-entity border-2 border-er-border rounded-xl shadow-2xl',
...
function ModalHeader({ onClose }: { onClose: () => void }) {
  return (
    <header className="flex items-center justify-between gap-4 px-6 py-4 bg-er-entity-header border-b border-er-border cursor-grab active:cursor-grabbing">
...
function ModalFooter({ onClose }: { onClose: () => void }) {
  return (
    <footer className="px-6 py-4 bg-er-entity-header border-t border-er-border">
      <button
        type="button"
        onClick={onClose}
        className="w-full px-4 py-2.5 text-sm font-medium text-er-pk-text bg-er-pk hover:bg-er-pk/80 rounded-lg transition-colors"
      >
        Got it!
      </button>
    </footer>
...
function InfoIcon() {
  return (
    <svg className="w-5 h-5 text-er-pk-text" viewBox="0 0 20 20" fill="currentColor">
...
function TablesSection() {
  return (
    <section>
      ...
      <div className="flex items-start gap-4 p-3 bg-er-entity-header/50 rounded-lg border border-er-border">
        <TablePreview />
        <div className="space-y-2 text-sm">
          ...
          <ul className="space-y-1 text-er-text-muted">
            <KeyLegendItem color="bg-er-pk" textColor="text-er-pk-text" label="Primary Key (PK)" description="Unique identifier" />
            <KeyLegendItem color="bg-er-fk" textColor="text-er-fk-text" label="Foreign Key (FK)" description="Links to another table" />
            <KeyLegendItem color="bg-er-uk" textColor="text-er-uk-text" label="Unique Key (UK)" description="Unique constraint" />
          </ul>
        </div>
      </div>
    </section>
...
function TablePreview() {
  return (
    <div className="shrink-0 w-36 rounded-lg border-2 border-er-border overflow-hidden bg-er-entity">
      <div className="bg-er-entity-header px-2 py-1.5 font-semibold text-er-text text-xs border-b border-er-border">
        users
      </div>
      <div className="text-xs">
        <div className="px-2 py-1 bg-er-pk flex items-center gap-2">
          <span className="text-er-pk-text font-medium">id</span>
          <span className="text-er-pk-text/60 ml-auto text-[10px]">PK</span>
        </div>
...
function RelationshipExample() {
  return (
    <div className="p-3 bg-er-entity-header/30 rounded-lg border border-er-border space-y-3">
      {/* Example 1 */}
      <div>
        <div className="flex items-center justify-between gap-2 mb-1">
          <div className="text-xs font-medium text-er-fk-text bg-er-fk px-2 py-0.5 rounded">senses.entry_id</div>
          <div className="text-xs font-medium text-er-pk-text bg-er-pk px-2 py-0.5 rounded">lexical_entries.id</div>
        </div>
        <svg className="w-full h-6" viewBox="0 0 200 24">
          <path d="M 22 6 L 10 12 L 22 18 Z" className="fill-er-relation" />
          <line x1="22" y1="12" x2="172" y2="12" className="stroke-er-relation" strokeWidth="2" />
          <circle cx="180" cy="12" r="5" className="fill-er-relation" />
        </svg>
      </div>
      {/* Example 2 */}
      <div>
        <div className="flex items-center justify-between gap-2 mb-1">
          <div className="text-xs font-medium text-er-fk-text bg-er-fk px-2 py-0.5 rounded">word_forms.entry_id</div>
          <div className="text-xs font-medium text-er-pk-text bg-er-pk px-2 py-0.5 rounded">lexical_entries.id</div>
        </div>
        <svg className="w-full h-6" viewBox="0 0 200 24">
          <path d="M 22 6 L 10 12 L 22 18 Z" className="fill-er-relation" />
...
function SymbolLegend({ type, label, description }: { type: 'fk' | 'pk'; label: string; description: string }) {
  return (
    <div className="flex items-center gap-2 p-2 bg-er-entity-header/20 rounded-lg">
      <svg className="w-10 h-6 shrink-0" viewBox="0 0 40 24">
        {type === 'fk' ? (
          <>
            {/* Arrow pointing in = "many" */}
            <path d="M 18 6 L 8 12 L 18 18 Z" className="fill-er-relation" />
            <line x1="18" y1="12" x2="34" y2="12" className="stroke-er-relation" strokeWidth="2" />
          </>
        ) : (
          <>
            {/* Circle = "one" */}
            <line x1="6" y1="12" x2="22" y2="12" className="stroke-er-relation" strokeWidth="2" />
            <circle cx="28" cy="12" r="5" className="fill-er-relation" />
          </>
        )}
      </svg>
      <div className="text-xs">
...
function InteractionItem({ label, description }: { label: string; description: string }) {
  return (
    <div className="p-2 bg-er-entity-header/20 rounded-lg">
      <p className="font-medium text-er-text">{label}</p>
      <p className="text-er-text-muted">{description}</p>
    </div>
  )
