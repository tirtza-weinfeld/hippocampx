...
function getContentClasses(card: CardType): string {
  if (card.matched) {
    return "text-game-success-text font-semibold text-xs sm:text-sm"
  }
  if (card.type === "word") {
    return "text-game-primary font-bold text-xs sm:text-sm"
  }
  return "text-game-text text-xs sm:text-sm leading-snug"
}

function getFrontClasses(card: CardType): string {
  if (card.matched) {
    return "bg-game-success-light border border-game-success"
  }
  return "bg-game-surface border border-game-primary shadow-sm"
}
  ...
  return (
    <div className="aspect-square [perspective:600px]">
      <motion.button
        ...
        className="relative w-full h-full cursor-pointer select-none focus:outline-none focus-visible:ring-2 focus-visible:ring-game-primary focus-visible:ring-offset-1 [transform-style:preserve-3d]"
        initial={false}
      ...
      >
        ...
        <div className="absolute inset-0 rounded-lg p-1.5 sm:p-2 flex items-center justify-center bg-game-gradient border border-transparent shadow-sm [backface-visibility:hidden]">
        ...
        <div className={`absolute inset-0 rounded-lg p-1 sm:p-1.5 flex flex-col items-center justify-center ${getFrontClasses(card)} [backface-visibility:hidden] [transform:rotateY(180deg)]`}>
          {card.matched && (
            <Sparkles className="h-3 w-3 text-game-success flex-shrink-0" />
          )}
