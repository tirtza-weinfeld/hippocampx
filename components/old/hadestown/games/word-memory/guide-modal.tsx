...
function MiniCard({ flipped = false, matched = false, content, isWord = false, className = "" }: MiniCardProps) {
  if (matched) {
    return (
      <div className={`w-9 h-12 rounded bg-old-game-success-light border border-old-game-success flex items-center justify-center ${className}`}>
        <Sparkles className="h-3 w-3 text-old-game-success" />
      </div>
    )
  }
  if (flipped && content) {
    return (
      <div className={`w-9 h-12 rounded bg-old-game-surface border-2 border-old-game-primary flex items-center justify-center p-0.5 ${className}`}>
        <span className={`text-[7px] text-center leading-tight ${isWord ? "text-old-game-primary font-bold" : "text-old-game-text"}`}>
          {content}
        </span>
      </div>
    )
  ...
  return (
    <div className={`w-9 h-12 rounded bg-old-game-gradient flex items-center justify-center shadow-sm ${className}`}>
  ...
  if (step === 0) {
    return (
      <div className="relative flex items-center justify-center">
        <motion.div
          className="absolute w-28 h-28 bg-old-game-primary/10 rounded-full blur-xl"
      ...
      <div className="relative flex items-center justify-center" style={{ perspective: "800px" }}>
          ...
          {[0, 1, 2, 3, 4, 5, 6, 7].map(function(i) {
            if (i === 5) {
              return (
                ...
                >
                  <div className="absolute inset-0 rounded bg-old-game-gradient flex items-center justify-center" style={{ backfaceVisibility: "hidden" }}>
                    <BookOpen className="h-4 w-4 text-white" />
                  </div>
                  <div className="absolute inset-0 rounded bg-old-game-surface border-2 border-old-game-primary flex items-center justify-center" style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}>
                    <span className="text-old-game-primary font-bold text-[8px]">Lyre</span>
                  </div>
                </motion.div>
              )
            }
        ...
        >
          <svg width="18" height="22" viewBox="0 0 24 24" fill="none">
            <path d="M5 3L19 12L12 13L9 20L5 3Z" fill="white" stroke="currentColor" strokeWidth="1.5" className="text-old-game-text" />
          </svg>
          <motion.div
            className="absolute top-0 left-0 w-3 h-3 bg-old-game-primary rounded-full"
  ...
  if (step === 2) {
    return (
      <div className="relative flex items-center justify-center" style={{ perspective: "800px" }}>
            ...
            if (i === 2) {
              return (
                ...
                >
                  ...
                  <div className="absolute inset-0 rounded bg-old-game-surface border-2 border-old-game-primary flex items-center justify-center" style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}>
                    <span className="text-old-game-primary font-bold text-[7px]">Echo</span>
                  </div>
                </motion.div>
              )
            }
            if (i === 5) {
              return (
                ...
                >
                  ...
                  <div className="absolute inset-0 rounded bg-old-game-surface border-2 border-old-game-primary flex items-center justify-center p-0.5" style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}>
                    <span className="text-old-game-text text-[6px] text-center leading-tight">Sound reflected</span>
                  </div>
                </motion.div>
              )
            }
        ...
        >
          <Sparkles className="h-8 w-8 text-old-game-primary" />
        </motion.div>
        <motion.div
          className="absolute pointer-events-none z-10"
          animate={shouldReduceMotion ? undefined : {
    ...
    return (
      <div className="relative flex items-center justify-center gap-4">
            ...
            if (i === 1 || i === 6) {
              return (
                ...
                >
                  ...
                  <div className="absolute inset-0 rounded bg-old-game-surface border-2 border-old-game-error flex items-center justify-center" style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}>
                    <span className="text-[6px] text-old-game-error">X</span>
                  </div>
                </motion.div>
              )
            }
        ...
        <div className="relative ml-2">
          <motion.div
            className="absolute -inset-2 rounded-full bg-old-game-primary/15"
          ...
          <motion.div
            ...
            transition={{ duration: 1.5, repeat: Infinity }}
          >
            <BrainIcon className="h-10 w-10 text-old-game-primary" />
          </motion.div>
        </div>
      </div>
    )
  ...
  if (step === 4) {
    return (
      <div className="relative flex items-center justify-center gap-4">
        ...
        >
          <Trophy className="h-12 w-12 text-old-game-primary" />
          <motion.div
            className="absolute -top-1 -right-1"
            animate={shouldReduceMotion ? undefined : { scale: [0.8, 1.2, 0.8], opacity: [0.6, 1, 0.6] }}
            transition={{ duration: 1.2, repeat: Infinity }}
          >
            <Sparkles className="h-4 w-4 text-old-game-primary" />
          </motion.div>
        </motion.div>
      </div>
    )
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          transition={{ duration: 0.2 }}
          className="bg-old-game-surface rounded-2xl p-6 w-[340px] h-[400px] shadow-2xl border border-old-game-border flex flex-col"
          onClick={handleModalClick}
        >
          <div className="flex justify-between items-start mb-3 flex-shrink-0">
            <h2 className="text-lg font-bold text-old-game-primary pr-4 line-clamp-1">
              {step.title}
            </h2>
            <button
              onClick={onClose}
              className="p-1 rounded-full text-old-game-text-muted hover:text-old-game-text hover:old-game-primary-soft transition-colors flex-shrink-0"
          ...
          <div className="flex-1 flex items-center justify-center px-2">
            <p className="text-old-game-text text-center leading-relaxed text-sm">
              {step.content}
            </p>
          </div>
              ...
              return (
                <div
                  ...
                  className={`h-1.5 rounded-full transition-all duration-200 ${
                    isActive
                      ? "w-4 bg-old-game-primary"
                      : "w-1.5 bg-old-game-border"
                  }`}
                />
              )
            })}
          ...
          <div className="flex justify-between items-center gap-3 flex-shrink-0">
            <button
              ...
              className="flex items-center gap-1 px-3 py-2 rounded-full border border-old-game-border text-sm text-old-game-text-muted hover:text-old-game-primary hover:border-old-game-primary disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
            ...
            <button
              onClick={onNext}
              className="flex items-center gap-1.5 px-5 py-2 bg-old-game-gradient text-white text-sm font-medium rounded-full shadow-md hover:shadow-lg transition-shadow"
            >
