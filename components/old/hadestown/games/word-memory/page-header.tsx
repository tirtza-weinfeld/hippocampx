...
export function PageHeader({ shouldReduceMotion }: PageHeaderProps) {
  return (
    <div className="text-center mb-8">
      ...
      >
        <div className="absolute inset-0 bg-gradient-to-r from-game-primary/20 via-game-primary-dark/20 to-game-primary/20 rounded-2xl blur-lg" />

        <div className="relative bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-2xl px-8 py-5 shadow-xl border border-game-border">
          <div className="flex items-center justify-center gap-4">
            <motion.div
              animate={shouldReduceMotion ? undefined : { rotate: [0, 8, 0, -8, 0] }}
              transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
            >
              <BrainIcon className="h-8 w-8 text-game-primary" />
            </motion.div>
            <h1 className="text-3xl md:text-4xl font-bold text-game-gradient">
              Word Memory
            </h1>
            <motion.div
              animate={shouldReduceMotion ? undefined : { rotate: [0, -8, 0, 8, 0] }}
