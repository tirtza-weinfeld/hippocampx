...
export function LyricChallengeGame({ lyrics, onComplete }: LyricChallengeGameProps) {
  ...
  return (
    <div className="space-y-6 bg-old-game-surface">
      {/* Header */}
      <div className="flex items-center gap-3">
        ...
        <motion.button
          ...
          className="p-2.5 rounded-full bg-old-game-surface border border-old-game-border text-old-game-text-muted hover:text-old-game-primary hover:border-old-game-primary transition-colors"
        >
        ...
        <div className="flex-1 space-y-1">
          <div className="flex justify-between text-xs">
            <span className="text-old-game-text-muted">Song {state.currentIndex + 1} of {totalLyrics}</span>
            <span className="text-old-game-primary font-semibold">{state.score} correct</span>
          </div>
      ...
      <div className="text-center">
        ...
        >
          <Music className="h-5 w-5 text-old-game-primary" />
          <h2 className="text-xl font-bold text-old-game-primary">{current.title}</h2>
          <Music className="h-5 w-5 text-old-game-primary" />
        </motion.div>
        ...
        <motion.button
          ...
          whileTap={shouldReduceMotion ? undefined : { scale: 0.95 }}
          className="flex items-center gap-1 mx-auto px-3 py-1 text-xs rounded-full bg-old-game-surface border border-old-game-border text-old-game-primary"
        >
        ...
        <AnimatePresence>
          {state.showHint && (
            <motion.p
              ...
              className="mt-3 p-3 rounded-xl bg-old-game-primary-soft text-old-game-text text-sm"
            >
              {current.context}
            </motion.p>
          )}
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="p-4 rounded-xl bg-old-game-surface border border-old-game-border space-y-3"
        >
          {current.lyrics.map((line, lineIndex) => {
            ...
            return (
              <div key={lineIndex} className="flex items-start gap-3">
                <div className="flex-shrink-0 w-6 h-6 rounded-full bg-old-game-primary-soft text-old-game-primary flex items-center justify-center font-medium text-sm">
                  {lineIndex + 1}
                </div>
                  ...
                  <div className="flex-1">
                    <input
                      ...
                      className={`
                        ...
                        bg-old-game-surface placeholder-old-game-text-muted
                        transition-colors focus:outline-none
                        ${state.isChecked
                          ? checkLyricMatch(state.userInputs[missingIndex] || "", line)
                            ? "border-old-game-success text-old-game-success-text"
                            : "border-old-game-error text-old-game-error-text"
                          : "border-old-game-border text-old-game-text focus:border-old-game-primary"
                        }
                      `}
                    />
                    {state.isChecked && (
                      ...
                      >
                        {checkLyricMatch(state.userInputs[missingIndex] || "", line) ? (
                          <Check className="h-3 w-3 text-old-game-success" />
                        ) : (
                          <X className="h-3 w-3 text-old-game-error" />
                        )}
                        <span className="text-old-game-text-muted">
                          Actual: <span className="text-old-game-text">{line}</span>
                        </span>
                      </motion.div>
                    )}
                  </div>
                ) : (
                  <p className="flex-1 px-3 py-2 rounded-lg bg-old-game-surface-soft text-old-game-text">
                    {line}
                  </p>
                )}
              </div>
        ...
        {!state.isChecked ? (
          ...
          <div className="flex gap-3">
            <motion.button
              ...
              className="flex-1 py-3 text-sm font-semibold rounded-xl bg-old-game-surface border border-old-game-border text-old-game-text-muted hover:text-old-game-primary hover:border-old-game-primary transition-colors"
            >
              Try Again
            </motion.button>
            <motion.button
              ...
              className={`flex-1 py-3 text-sm font-semibold rounded-xl text-white shadow-md flex items-center justify-center gap-2 ${
                allCorrect ? "bg-gradient-to-r from-old-game-success to-emerald-600" : "bg-old-game-gradient"
              }`}
            >
