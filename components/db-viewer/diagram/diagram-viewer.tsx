...
export function DiagramViewer({ topology }: DiagramViewerProps) {
  ...
  function handleTableSelect(tableName: string) {
    router.push(`/db/tables?table=${tableName}`);
  }

  return (
    <div className="relative size-full">
      <ERDiagram topology={topology} onTableSelect={handleTableSelect} />

      {/* Navigation Header */}
      <div className="absolute top-4 right-4 z-10 flex items-center gap-2">
        <Link
          href="/db/tables"
          className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium bg-er-control border border-er-border text-er-text hover:bg-er-control-hover transition-colors shadow-sm"
        >
